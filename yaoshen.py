...
def getHttpProxy(n):
    print(n)
    resp = requests.get(f"http://dev.kdlapi.com/api/getproxy/?orderid=953671667394527&num={n}&protocol=2&method=1&an_an=1&quality=0&sep=1").text
    proxys = resp.split("|")
    proxies = [{
        'http': 'http://' + proxy,
        'https': 'https://' + proxy
...
def ThreadGetinfo(s,e):
    ## 爬取数据，存在大量重复数据，需要多次执行，最后统一去重
    global yaoshen
    proxies = getHttpProxy(int((e-1-s)/15))
    j = 0
    for i in range(s, e, 15):
        # time.sleep(2)

        url = 'https://m.maoyan.com/mmdb/comments/movie/1200486.json?_v_=yes&offset=' + str(i)
        print(url)
        html = requests.get(url=url, cookies=cookie, headers=header, proxies=proxies[j])
        print(html)
        try:
            html = html.json()
        ...
        #                       'nick': item['nick']}, ignore_index=True)
        # print(yaoshen)
        yaoshen.to_excel('我不是药神_mul_threading.xlsx', index=False)
        if j > len(proxies):
            j=len(proxies)-1
        else:
            j = j +1
